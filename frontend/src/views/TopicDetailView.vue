...
<template>
  ...
  <div v-else-if="topic" class="space-y-8">
    <div class="panel-warning flex flex-col md:flex-row flex-wrap items-center justify-center md:justify-between gap-6 bg-[#fcd34d] p-6 rounded-2xl border-[3px] border-black shadow-[6px_6px_0px_0px_#000]">
      ...
      <div class="flex flex-wrap justify-center gap-2 md:gap-3 w-full md:w-auto">
        <button
          type="button"
          class="topic-status-pill rounded-xl border-[3px] border-black px-4 py-2 font-display font-bold text-sm transition-all shadow-[4px_4px_0px_0px_#000] hover:-translate-y-[2px] hover:-translate-x-[2px] active:translate-y-0 active:translate-x-0 active:shadow-[0px_0px_0px_0px_#000]"
        ...
        <button
          type="button"
          class="topic-status-pill rounded-xl border-[3px] border-black px-4 py-2 font-display font-bold text-sm transition-all shadow-[4px_4px_0px_0px_#000] bg-white hover:bg-[#a5f3fc] hover:-translate-y-[2px] hover:-translate-x-[2px] active:translate-y-0 active:translate-x-0 active:shadow-[0px_0px_0px_0px_#000]"
    ...
    <div class="panel-secondary rounded-2xl border-[3px] border-black bg-[#e0e7ff] p-6 md:p-8 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
      ...
      <div class="space-y-4">
        <div v-for="note in notes" :key="note.id" class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 rounded-xl border-[3px] border-black bg-white p-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
          <template v-if="editingNoteId === note.id">
            ...
            <button type="button" class="w-full sm:w-auto rounded-lg border-2 border-black bg-[#4ade80] text-black px-4 py-2 font-display font-bold text-sm transition-all shadow-[2px_2px_0px_0px_#000] active:translate-y-[2px] active:shadow-none" @click="saveNote">Зберегти</button>
          </template>
          <template v-else>
            <p class="min-w-0 w-full sm:flex-1 text-base font-medium">{{ note.text }}</p>
            <div class="flex flex-wrap gap-2 w-full sm:w-auto">
              <button type="button" class="rounded-lg border-2 border-black bg-[#fef08a] text-black px-3 py-1 font-display font-bold text-xs transition-all shadow-[2px_2px_0px_0px_#000] active:translate-y-[2px] active:translate-x-[2px] active:shadow-none" @click="startEdit(note)">Редаг.</button>
              <button type="button" class="rounded-lg border-2 border-black bg-[#fca5a5] text-black px-3 py-1 font-display font-bold text-xs transition-all shadow-[2px_2px_0px_0px_#000] active:translate-y-[2px] active:translate-x-[2px] active:shadow-none" @click="deleteNote(note.id)">Видал.</button>
            </div>
          </template>
        </div>
      </div>

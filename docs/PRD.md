...
- Структуроване вивчення тем за категоріями
...
- Категорії з темами для підготовки до співбесід
...
| DELETE | `/api/topics/notes/<id>/delete/` | Видалити замітку | Yes |

### Trainer

| Method | Path | Description | Auth |
|--------|------|-------------|------|
| POST | `/api/trainer/sessions/` | Створити сесію (category, total_questions) | Yes |
...
При імпорті: статуси оновлюються через merge/upsert, а замітки для імпортованих тем
повністю замінюються даними з JSON-файлу.
...
### Login / Register
Окремі сторінки входу та реєстрації з двома полями (email, password) і посиланням
для переходу між ними.

### Dashboard
Карточки категорій з progress bar (% вивчено), лічильниками вивчених/на повторення
та кнопками експорту/імпорту прогресу.

### Topics List
Ліва панель — список категорій. Основна область — карточки тем зі статусом
(нова/вивчена/повторити), пошуком, фільтром за статусом і пагінацією.

### Topic Detail
Заголовок теми, кнопки статусу "Вивчив" / "На повторення" / "Нова". Два розділи:
коротка відповідь та детальне пояснення з підсвіткою коду. Внизу — секція заміток.

### Trainer Setup
Вибір категорії через dropdown, числове поле для кількості питань (1-50) і кнопка
"Почати".
...
### Trainer Results
Сторінка з copyable textarea для ChatGPT-промпту та кнопкою "Скопіювати промпт".
